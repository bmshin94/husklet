...
fn request_capability(request: &str) -> Capability {
    match request {
        ...
        "workspace_create" => Capability::WorkspaceCreate,
        "workspace_delete" => Capability::WorkspaceRemove,
        "workspace_start" | "workspace_stop" | "workspace_restart" => Capability::WorkspaceLifecycle,
        ...
        "pane_semantic_action" | "pane_semantic_action_once" => Capability::PaneSemanticControl,
...
pub fn document() -> String {
    ...
    assert_eq!(
        REQUEST_TO_REPLY
            .iter()
            .map(|(request, _)| (*request).to_owned())
            .collect::<Vec<_>>(),
        request_variants,
    ...
    assert!(
        REQUEST_TO_REPLY
            .iter()
            .all(|(_, reply)| reply_variants.iter().any(|candidate| candidate == reply)),
        "REQUEST_TO_REPLY names an absent Reply"
    );
    assert_eq!(
