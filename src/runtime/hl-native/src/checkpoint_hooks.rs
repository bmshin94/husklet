...
#[cfg(feature = "native-test-hooks")]
pub(crate) fn checkpoint_external_preflight_key_test(isa: u32, scenario: u32) -> Result<(), i32> {
    scenario_status(
        isa,
        test_api().aarch64_checkpoint_external_preflight_key,
        test_api().x86_64_checkpoint_external_preflight_key,
        scenario,
    )
}
