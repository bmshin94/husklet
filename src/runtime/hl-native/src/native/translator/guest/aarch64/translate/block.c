...
static enum translation_step translate_system_instruction(uint64_t guest_pc, uint32_t instruction) {
    ...
    if ((instruction & 0xFFFFFFE0u) == 0xD50B7420u) {
        ...
        // One lowering for the window test, shared with every other folded access: it parks the
        // guest flags in the CPU record rather than in host x18, which Darwin reserves and clears
        // asynchronously (a flag word left there returns as zero and silently rewrites NZCV).
