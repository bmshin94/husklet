...
static int ckpt_stdio_alias_test_install(struct ckpt_stdio_alias_test_box *fixture, int native_fd, int guest_fd,
    ...
    hl_host_result imported =
        hl_c_bridge_host_import_file((hl_c_bridge_host *)fixture->host, native_fd,
                                     (status_flags & O_ACCMODE) == O_RDONLY ? HL_HOST_FILE_READ : HL_HOST_FILE_WRITE);
...
release:
    for (int fd = 0; fd <= 11; ++fd)
        (void)hl_linux_fd_close(&fixture.box, (hl_linux_fd)fd, NULL);
...
static int ckpt_register_ready(struct cpu **live, int count) {
    ...
    if (status < 0) {
        ...
        fprintf(stderr,
                "[ckpt] refuse: REGISTER_READY for host process %d was refused by the broker (status %d, "
                "member %llu)\n",
...
static int ckpt_dump_self_locked(struct cpu *c, const char *group) {
    ...
    if (ckpt_self_identity(&m, ckpt_group_gpid(group)) != 0)
        CKPT_DUMP_FAIL("self identity"); // common cleanup: frees fdrecs and reports the abort
...
static void ckpt_exemption_responder(hl_activation_descriptor broker, long long expected, uint64_t answer) {
    ...
    if (read((int)channel, &request, sizeof request) == (ssize_t)sizeof request &&
        ...
        request.op == HL_CKPT_OP_PARTICIPANT_REGISTERED && request.name_size == 0 && request.length == sizeof named &&
        read((int)channel, &named, sizeof named) == (ssize_t)sizeof named && named == (uint64_t)expected) {
...
HL_API int HL_TARGET_LOCAL(checkpoint_rendezvous_test)(uint32_t scenario) {
    ...
    if (scenario >= 4) return ckpt_outstanding_test(scenario);
