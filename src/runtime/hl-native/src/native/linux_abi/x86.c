...
const char *g_pcache_authorized_guest_path;

static int x86_pcache_hex(char value) {
...
static int x86_pcache_authority(uint64_t size, hl_identity_digest *digest) {
    ...
    for (const char *record = records; *record;) {
        ...
        if (first != NULL && second != NULL && third != NULL && path_size == strlen(path) &&
            memcmp(first + 1, path, path_size) == 0 && end - third - 1 == 64) {
            char number[32];
            ...
            for (size_t index = 0; index < sizeof digest->bytes; index++) {
                int high = x86_pcache_hex(third[1 + index * 2]);
                int low = x86_pcache_hex(third[2 + index * 2]);
