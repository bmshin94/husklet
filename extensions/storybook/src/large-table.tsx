...
export class LargeRecordSource {
  ...
  answer(value: unknown) {
    ...
    const count = Math.min(
      request.range.count,
      WINDOW_LIMIT,
      Math.max(0, this.length() - request.range.start),
    );
    const rows = Array.from({ length: count }, (_, offset) =>
      this.row(request.range.start + offset),
    );
      ...
      cells: [
        ...
        { Bytes: 65_536 + logical * 128 },
        {
          Badge: {
            ...
            tone: logical % 3 ? 'Positive' : 'Warning',
          },
        },
      ],
    };
...
export function LargeDataTableStory({ source }: { source: LargeRecordSource }) {
  ...
  const update = (next: Partial<SourceConfiguration>) => {
    ...
    setFilter(changed.filter);
    setDescending(changed.descending);
      ...
      {interactions.length === 0
        ? [
            <InlineMessage
              ...
              label={'Focus a control or select a record to inspect its bounded event history.'}
              tone={'neutral'}
            />,
          ]
