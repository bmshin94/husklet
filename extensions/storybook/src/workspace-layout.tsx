...
export const retainEvents = (events: readonly string[], next: string): string[] =>
  [...events, next].slice(-EVENT_LIMIT);
...
const initial = boundedPanes([
  ...
  {
    slot: 'pane-extension-2',
    title: 'Container operations',
    occupant: 'extension',
    provider: 'top/containers',
    tab: 'shells',
  },
...
export function WorkspaceLayoutStory() {
  ...
  const [events, setEvents] = useState<string[]>([
    'Ready: shells active; pane-terminal-1 focused.',
  ]);
  ...
  const split = (nextOrientation: SplitOrientation) => {
    ...
    const pane: BoundedPane = {
      slot: `pane-new-${panes.length + 1}`,
      title: 'New terminal',
      occupant: 'terminal',
      tab: activeTab,
      provider: '',
    ...
    record(
      `Split ${selected.slot} ${nextOrientation === 'horizontal' ? 'beside' : 'below'} into ${pane.slot}.`,
    );
  };
  ...
  return (
    <Scroll width={'fill'} height={'fill'}>
      <Column gap={2} width={'fill'}>
        ...
        <Text
          label={
            'Tabs, nested splits, keyboard focus, and chooser occupant changes retain immutable pane slots.'
          }
          wrap={true}
        />
        ...
        <Row gap={2} wrap={true}>
          ...
          <Button
            ...
            onInvoke={() => {
              setFocusedSlot(selected.slot);
              record(`Focused ${selected.slot} in ${activeTab}.`);
            }}
          />
          <Button
        ...
        <Column gap={1}>
          ...
          <Text label={`${events.length}/${EVENT_LIMIT} recent events`} color={'text-dim'} />
          {events.map((event, index) => (
            <Text key={`${index}:${event}`} label={event} wrap={true} />
          ))}
        </Column>
      </Column>
    </Scroll>
