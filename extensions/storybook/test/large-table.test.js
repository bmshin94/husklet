...
test('one DataTable node represents one million rows without materializing row nodes', () => {
  const stage = host();
  const source = new LargeRecordSource();
  const frame = stage.render(h(LargeDataTableStory, { source }));
  const created = frame.patches
    .filter((patch) => 'Create' in patch)
...
test('sort, filter, resize and all states remain bounded source operations', async () => {
  ...
  const request = {
    ...
    source: SOURCE,
    version: 1,
    range: { start: 40_000, count: 10_000 },
    sort: null,
    filter: null,
  };
  ...
  assert.equal(
    first.rows.length,
    WINDOW_LIMIT,
    'a resized viewport cannot exceed one protocol window',
  );
  ...
  assert.deepEqual(
    await source.sort({ source: SOURCE, version: 1, column: 'id', descending: true }),
    { accepted: true },
  );
  ...
  assert.equal(
    ...
    null,
    'a stale pre-sort request cannot repopulate the table',
  );
...
test('selection requires immutable current-generation authority and remains bounded', () => {
  ...
  assert.ok(
    labels.includes(`Recent operations (${OPERATION_HISTORY_LIMIT}/${OPERATION_HISTORY_LIMIT})`),
  );
  const final = stage.frames.at(-1).patches.filter((patch) => patch.SetProp?.prop === 'Label');
  assert(
    final.length <= OPERATION_HISTORY_LIMIT + 1,
    'one event must not materialize an unbounded detail list',
  );
  assert(
    !JSON.stringify(stage.frames).includes('1041'),
    'selection details must not serialize all selected rows',
  );
...
test('edits bind immutable row and source version, validate, and advance the version', async () => {
  ...
  assert.deepEqual(
    ...
    { accepted: false, reason: 'stale version' },
  );
  assert.equal(source.row(42).cells[1].Text, 'renamed');
  assert.deepEqual(
    await source.edit({
...
test('native sort proposals bind source version and refuse stale or unsortable authority', async () => {
  ...
  const source = new LargeRecordSource(async (call, argument) => mutations.push([call, argument]));
  assert.deepEqual(
    await source.sort({ source: SOURCE, version: 1, column: 'name', descending: true }),
    { accepted: true },
  );
  assert.equal(source.version, 2);
  assert.equal(source.descending, true);
  assert.equal(mutations.at(-1)[1].mutation.Length.version, 2);
  assert.deepEqual(
    await source.sort({ source: SOURCE, version: 1, column: 'name', descending: false }),
    { accepted: false, reason: 'stale version' },
  );
  assert.deepEqual(
    await source.sort({ source: SOURCE, version: 2, column: 'state', descending: false }),
    { accepted: false, reason: 'unsortable column' },
  );
  assert.equal(source.version, 2);
  assert.equal(source.descending, true);
});

test('the rendered million-row table exposes only the declared editable column', () => {
  const stage = host();
  const source = new LargeRecordSource();
  const frame = stage.render(h(LargeDataTableStory, { source }));
  const schema = frame.patches.find((patch) => patch.SetProp?.prop === 'Schema').SetProp.value
    .Schema;
  assert.deepEqual(
    schema.filter((column) => column.editable).map((column) => column.key),
    ['name'],
  );
});
...
test('the high-density operations story is selectable in the shipped playground', () => {
  ...
  const selector = first.patches.find((patch) => patch.Create?.tag === 'Autocomplete').Create.id;
  ...
  stage.surface.dispatch({
    ...
    node: selector,
    id: `${selector}:Select`,
    rows: [choices.findIndex((choice) => choice.value === 'DataTable')],
  });
  assert.ok(node(stage.since(before), 'Heading', '1,000,000 logical records'));
});
