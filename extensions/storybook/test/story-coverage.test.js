...
import {
  ...
  boundedInstructions,
  INSTRUCTION_LIMIT,
} from '../dist/disassembly-inspection.js';
import {
...
import {
  ...
  COVERAGE_LIMIT,
  SOURCE_LIMIT,
} from '../dist/coverage-inspection.js';
import {
  NetworkWaterfallStory,
  boundedRequests,
  REQUEST_LIMIT,
  PHASE_LIMIT,
} from '../dist/network-waterfall.js';
...
import {
  ...
  PROCESS_LIMIT,
  LOG_LIMIT,
} from '../dist/container-operations.js';
import {
...
import {
  ...
  GRANT_LIMIT,
  FIELD_LIMIT,
} from '../dist/extension-lifecycle.js';
import {
  ...
  PATH_LIMIT,
  CONTENT_LIMIT,
} from '../dist/workspace-file-edit.js';
import {
  ImagePullStory,
  boundedPull,
  LAYER_LIMIT,
...
test('every catalogue contract has a meaningful selectable state and family coverage', () => {
  ...
  for (const story of coverage.componentStories) {
    assert(
      story.state.children.length > 0 || Object.keys(story.state.props).length > 0,
...
test('every composed story has a readable root and a bounded initial wire frame', () => {
  ...
  for (const [name, story] of stories) {
    ...
    assert(
      frame.patches.some((patch) => patch.Create?.tag === 'Heading'),
      `${name} has no semantic heading root`,
    );
    assert(
...
test('container operations bounds and sanitizes every host-sized projection', () => {
  const containers = Array.from({ length: CONTAINER_LIMIT + 3 }, (_, index) => ({
    ...
    processes: Array.from({ length: PROCESS_LIMIT + 4 }, (_, pid) => ({
      pid,
      user: 'user',
...
test('workspace layout bounds slots and interactively splits by stable identity', () => {
  ...
  assert(
    changed.some(
      (patch) => patch.SetProp?.value?.Text === 'Split pane-terminal-1 below into pane-new-4.',
    ),
  );
...
test('extension lifecycle bounds authority and controls the selected immutable generation', () => {
  const source = Array.from({ length: EXTENSION_LIMIT + 3 }, (_, index) => ({
    ...
    version: 'v'.repeat(FIELD_LIMIT + 10),
    digest: `sha256:generation-${index}`,
    status: 'invented',
    grants: Array.from({ length: GRANT_LIMIT + 4 }, (_, grant) => `grant-${grant}`),
  }));
  ...
  assert(
    changed.some(
      (patch) => patch.SetProp?.value?.Text === 'Stopped top at sha256:top-generation-14.',
    ),
  );
});
...
test('workspace file review independently bounds paths, content, and interactive writes', () => {
  ...
  assert(
    changed.some(
      (patch) => patch.SetProp?.value?.Text === 'Renamed src/server.js to src/server.review.js.',
    ),
  );
  assert(
    changed.some((patch) => patch.SetProp?.value?.Text?.startsWith('src/server.review.js · ')),
  );
});
...
test('image pull independently bounds progress, target, digest, error, and cancellation', () => {
  const pull = boundedPull({
    job: '42',
    reference: 'r'.repeat(REFERENCE_LIMIT + 9),
    platform: 'linux/s390x',
    state: 'invented',
    digest: 'sha256:short',
    error: `failure\n${'e'.repeat(STATUS_LIMIT + 9)}`,
    layers: Array.from({ length: LAYER_LIMIT + 4 }, (_, index) => ({
      id: `layer-${index}`,
      current: 20,
      total: 10,
    })),
  ...
  assert(
    changed.some(
      (patch) =>
        ...
        'Cancelled image-pull job 42; the existing local image is unchanged.',
    ),
  );
  assert(!changed.some((patch) => patch.SetProp?.value?.Text?.startsWith('sha256:')));
});
test('dependency graph bounds and interactively filters issues', () => {
  const graph = boundedGraph({
    nodes: Array.from({ length: NODE_LIMIT + 2 }, (_, i) => ({
      id: `n${i}`,
      label: `n${i}`,
      version: '1',
      state: i ? 'resolved' : 'conflict',
      detail: 'x',
    })),
    edges: [],
    cycles: [],
    totals: { nodes: 99, edges: 0, cycles: 0 },
  });
  assert.equal(graph.nodes.length, NODE_LIMIT);
  assert.deepEqual(boundedGraph(null), {
    nodes: [],
    edges: [],
    cycles: [],
    totals: { nodes: 0, edges: 0, cycles: 0 },
  });
  const malformed = boundedGraph({
    nodes: [null, { id: 'a' }, { id: 'a' }],
    edges: [null, { source: 'a', target: 'missing' }],
    cycles: [[null]],
    totals: { nodes: -1 },
  });
  assert.equal(malformed.nodes.length, 1);
  assert.deepEqual(malformed.edges, []);
  assert.deepEqual(malformed.cycles, []);
  assert.equal(malformed.totals.nodes, 1);
  const stage = host();
  const first = stage.render(h(DependencyGraphStory));
  const filter = node(first.patches, 'Button', 'Show issues only');
  const before = stage.frames.length;
  assert(stage.surface.dispatch({ trigger: 'Invoke', node: filter, id: `${filter}:Invoke` }));
  const changed = stage.since(before);
  assert(changed.some((p) => p.Remove));
  assert(changed.some((p) => p.SetProp?.value?.Text === 'Show all'));
});

test('network waterfall validates, caps, sanitizes, and exposes typed hierarchy', () => {
  const phases = Array.from({ length: PHASE_LIMIT }, (_, i) => ({
    kind: 'wait',
    offsetUs: i * 2,
    durationUs: 2,
  }));
  const requests = Array.from({ length: REQUEST_LIMIT + 3 }, (_, i) => ({
    method: 'GET',
  ...
  requests.push({ ...requests[0], method: 'TRACE' });
  requests.push({
    ...requests[0],
    phases: [
      { kind: 'wait', offsetUs: 4, durationUs: 4 },
      { kind: 'dns', offsetUs: 2, durationUs: 3 },
    ],
  });
  const bounded = boundedRequests(requests, 90);
  assert.equal(bounded.requests.length, REQUEST_LIMIT);
  assert.equal(bounded.total, 90);
  assert.deepEqual(boundedRequests([null, { ...requests[0], phases: [null] }], Number.NaN), {
    requests: [],
    total: 0,
  });
  ...
  const stage = host();
  const initial = stage.render(h(NetworkWaterfallStory));
  const filter = node(initial.patches, 'Button', 'Show failures only');
  assert(filter);
  const before = stage.frames.length;
  assert(stage.surface.dispatch({ trigger: 'Invoke', node: filter, id: `${filter}:Invoke` }));
  const changed = stage.since(before);
  assert(
    changed.some(
      (p) => p.SetProp?.prop === 'Label' && p.SetProp.value.Text === 'Show all requests',
    ),
  );
  assert(
    changed.some((p) => p.Remove),
    'filtering must remove successful requests',
  );
});

test('coverage inspection bounds rows and source independently with visible truncation', () => {
  const lines = Array.from({ length: COVERAGE_LIMIT + 4 }, (_, index) => ({
    line: index + 1,
    hits: index % 2,
    source: 'x'.repeat(SOURCE_LIMIT + 9),
  }));
  const value = boundedCoverage(lines, 900);
  assert.equal(value.split('\n').length, COVERAGE_LIMIT + 1);
  assert.equal(value.split('\n')[0].split('\t')[2].length, SOURCE_LIMIT);
  assert(value.endsWith(`… showing ${COVERAGE_LIMIT} of 900 lines …`));
...
test('timeline view rejects blank events and enforces its hard ceiling', () => {
  const events = Array.from({ length: TIMELINE_LIMIT + 5 }, (_, index) => ({
    timestampMs: index,
    category: 'runtime',
    label: `event-${index}`,
    detail: 'observed',
  ...
  const value = boundedEvents(events);
  assert.equal(value.split('\n').length, TIMELINE_LIMIT);
  assert(!value.includes('blank'));
  assert(value.startsWith('0\truntime\tevent-0\tobserved'));
});

test('disassembly inspection rejects invalid instructions and enforces its hard ceiling', () => {
  const instructions = Array.from({ length: INSTRUCTION_LIMIT + 7 }, (_, index) => ({
    address: index,
    bytes: [0xc3],
    mnemonic: 'ret',
    operands: '',
  ...
  instructions.splice(
    ...
    0,
    null,
    { address: 2, bytes: [], mnemonic: 'bad', operands: '' },
    { address: 3, bytes: [256], mnemonic: 'wide', operands: '' },
  );
...
test('memory inspection rejects invalid regions and enforces its hard ceiling', () => {
  const regions = Array.from({ length: REGION_LIMIT + 9 }, (_, index) => ({
    start: index * 4096,
    end: (index + 1) * 4096,
    permissions: 'r-xp',
...
test('profile inspection rejects invalid frames and enforces its hard ceiling', () => {
  const frames = Array.from({ length: FRAME_LIMIT + 17 }, (_, index) => ({
    label: `frame-${index}`,
    samples: index + 1,
  }));
