...
test('extension acquisition preserves job revision and explicit grant identity', async () => {
  ...
  const operations = [
    ...
    api.extensions.install(
      ...
      { read: ['database.password'], write: [], expose_to_execution: ['database.password'] },
    ),
...
test('filesystem controls use exact confined protocol request shapes', async () => {
  ...
  assert.deepEqual((await next()).payload, {
    call: 'filesystem_read_link',
    with: { path: 'logs/current' },
  });
  assert.deepEqual((await next()).payload, {
  ...
  stage.host.write(
    encode({
      channel: 2,
      kind: KIND.response,
      payload: { reply: 'contents', with: [116, 97, 114, 103, 101, 116] },
    }),
  );
  for (let index = 2; index < operations.length; index += 1) {
    stage.host.write(encode({ channel: 2, kind: KIND.response, payload: { reply: 'done' } }));
  }
  const results = await Promise.all(operations);
  assert.equal(results[0].size, 4);
  assert.deepEqual(results[1], [116, 97, 114, 103, 101, 116]);
  stage.session.close();
  stage.host.destroy();
  stage.server.close();
});
