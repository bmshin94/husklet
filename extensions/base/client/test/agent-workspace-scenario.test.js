...
test('packaged external agent arms every workspace lifecycle mutation over Unix framing', async () => {
    ...
    socket.on('data', (chunk) => {
      for (const frame of reader.take(chunk)) {
        ...
        if (call === 'event_subscribe' || call === 'event_unsubscribe') {
          ...
          if (call === 'event_subscribe' && ++subscriptions > 1)
            socket.write(
              encode({
                channel: 250 + subscriptions,
                kind: KIND.event,
                payload: {
                  snapshot: 'workspace_lifecycle',
                  of: {
                    workspace: name,
                    action: ['create', 'start', 'stop', 'remove'][subscriptions - 1],
                    revision,
                    coalesced: 0,
                  },
                },
            ...
            encode({
              ...
              payload:
                call === 'workspace_create'
                  ? { reply: 'workspace_configuration', with: created }
                  : { reply: 'done' },
            }),
          );
        }
  ...
  try {
    const child = spawn(
      ...
      [
        example,
        JSON.stringify({ path: socketPath, name, image: 'alpine:3.20', architecture: 'amd64' }),
      ],
    ...
    assert.deepEqual(JSON.parse(stdout), { name, generation, revision: 44 });
