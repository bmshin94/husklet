...
test('packaged external agent controls terminal bytes and semantic UI over real Unix framing', async () => {
    ...
    socket.on('data', (chunk) => {
      for (const frame of reader.take(chunk)) {
        ...
        if (call === 'event_subscribe' || call === 'event_unsubscribe') {
          ...
          terminalReads += 1;
          const revision = terminalReads < 3 ? 7 : 8;
          socket.write(
            encode({
              channel: 2,
              kind: KIND.response,
        ...
        } else if (call === 'terminal_input_open') {
          socket.write(
            encode({
              channel: 2,
              kind: KIND.response,
          ...
          socket.write(
            encode({
              ...
              payload: {
                reply: 'semantics',
                with: semantic(revision, revision === 4 ? 'off' : 'on'),
              },
            }),
          );
        } else if (call === 'pane_semantic_action_once') {
          assert.equal(frame.payload.with.slot, 'ui-1');
  ...
  try {
    ...
    assert.equal(result.terminalAfter, '$ ^C');
    assert.match(result.ui, /<value>off<\/value>/);
    assert.match(result.uiAfter, /<value>on<\/value>/);
