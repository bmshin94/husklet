...
const mutate = async (action, authority) => {
  ...
  const event = new Promise((resolve) => {
    observed = (change) => {
      if (
        change.workspace === input.name &&
        change.action === action &&
        change.revision > lastRevision
      )
        resolve(change);
    };
  });
  ...
  try {
    ...
    const change = await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`${action} lifecycle event timed out`)), 1_000);
      }),
    ]);
...
try {
  ...
  await mutate('start', () => host.start(input.name, created.generation));
  running = true;
  await mutate('stop', () => host.stop(input.name, created.generation));
  running = false;
  await mutate('remove', () => host.delete(input.name, created.generation));
  removed = true;
  process.stdout.write(
    `${JSON.stringify({ name: created.name, generation: created.generation, revision: lastRevision })}\n`,
  );
} finally {
  try {
    if (created && !removed) {
      if (running) await host.stop(input.name, created.generation).catch(() => {});
