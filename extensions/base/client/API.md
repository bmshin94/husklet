...
- `host.containers.processes(...)` — `container_processes`, requires `containers:read`.
...
- `host.terminal.writeInput(...)` — `terminal_input_open`, requires `terminals:input`.
...
- `host.terminal.commandText(...)` interrupts in-flight start, input, output, and wait calls as well as idle polling immediately on abort, then cancels the exact owned command when the ordered connection remains usable. If abort closes an ambiguous connection, its `TerminalCommandOperationError` carries a frozen, JSON-safe `resume` token retaining the immutable command, exact acknowledged output cursor and bytes, and original aggregate byte and page ceilings across every reconnect; pass it directly to `resumeCommandText(...)`. Empty output pages consume that retained budget, and a caller may tighten but never widen it while resuming. A signal passed while resuming also cancels that recovered command, using the optional `cancelSignal` and `cancelTimeoutMs`, so a disconnected agent deadline cannot orphan host work.
...
- `host.files.walk(...)` — traverses directory trees through identity-pinned bounded pages with consumer backpressure, memory proportional to active depth, and caller-owned entry, page, and depth ceilings; rejects duplicate or backward host cursors; requires `filesystem:read`.
- `host.files.resumeWalk(errorOrToken)` — continues a disconnected recursive walk from a JSON-persistable, identity-pinned per-directory stack without replaying entries or resetting its spent work budget; a replaced directory fails closed, and recovery may tighten but never widen the retained bounds.
...
- `host.state.updateJson(codec, update, { attempts, signal })` — retries only CAS conflicts (up to 16 attempts); `update` may run more than once and must be safe to repeat. Abort interrupts in-flight reads and CAS writes; an ambiguous write remains recoverable by its exact `StateWriteOperationError` authority.
...
- `host.postgres.catalogueStartOnce(...)` — `postgres_catalogue_start_once`, requires `postgres:read`.
...
- `host.postgres.pages(..., { maxPages })` exposes a caller-owned total request ceiling (4,096 by default, including empty pages), rejects column-schema changes and cursor cycles, and preserves the bound plus an exact reconnect token in `PostgresPagesOperationError`. `resumePages(..., { maxPages })` may tighten but never widen or reset that retained budget.
...
- `host.volumes.list(...)` — `volume_list`, requires `volumes:read`.
- `host.volumes.inspect(...)` — `volume_inspect`, requires `volumes:read`.
- `host.volumes.create(...)` — `volume_create`, requires `volumes:write`.
- `host.volumes.remove(...)` — `volume_remove`, requires `volumes:write`.
...
- `host.subscribe('containers')` / `host.unsubscribe('containers')` — requires `containers:read`.
