...
export class PostgresPageProtocolError extends Error {
    ...
    constructor(lease, query, cursor, receivedLease, receivedQuery, receivedCursor) {
        super(`postgres page receipt does not match lease ${lease}, query ${query}, and its requested cursor`);
...
export class PostgresPagesOperationError extends Error {
    ...
    constructor(resume, cause) {
        super(`postgres page stream may resume at ${resume.cursor ?? 'the first page'}: ${cause instanceof Error ? cause.message : String(cause)}`, { cause });
...
export class PostgresCloseOperationError extends Error {
    ...
    constructor(recovery, cause) {
        super(`postgres ${recovery.kind} close may have committed: ${cause instanceof Error ? cause.message : String(cause)}`, { cause });
...
export function workspace(session, { signal } = {}) {
    ...
    const allocateTerminalInput = async (writer, sequence) => {
        ...
        if (!terminalInputWriter) {
            ...
            const retained = opening.catch((cause) => {
                if (terminalInputWriter === retained)
    ...
    const retireTerminalInput = async (writer) => {
    ...
    const exactWalkResume = (candidate) => {
        if (candidate === null ||
    ...
    const walkFrom = async function* (token, pageSize, walkSignal) {
        const stack = token.stack.map((frame) => ({
            path: frame.path,
            after: frame.after,
        ...
        while (stack.length > 0) {
            ...
            if (current.index < current.entries.length) {
                ...
                if (entries >= maxEntries)
                    throw new FileWalkLimitError('entries', recovery());
            ...
            if (pages >= maxPages)
                throw new FileWalkLimitError('pages', recovery());
            let page;
            try {
                ...
                if (cause instanceof DirectoryIdentityChangedError ||
    ...
    const api = {
        ...
        containers: {
            ...
            createOnce: async (token, configuration) => {
                ...
                const receipt = expect(await session.call('container_create_once', { token, spec: normalized }), 'container_create_once');
            ...
            pull: async (reference, { timeoutMs = 120_000, signal } = {}) => {
                ...
                try {
                    for (;;) {
                        ...
                        if (status.job !== job)
                            throw new ImagePullStatusProtocolError({ job }, status);
        ...
        terminal: {
            ...
            openTabOnce: async (token, title) => {
                const receipt = expect(await session.call('terminal_open_tab_once', { token, title }), 'terminal_open_tab_once');
                if (receipt.token !== token)
            ...
            writeObserved: (before, input, options) => {
                ...
                const contents = exactPaneInput(input);
                return api.terminal.writeInput(before.slot, before.generation, before.revision, contents, options);
            },
        ...
        files: {
            ...
            scopeChanges: (page, roots) => {
                ...
                const changes = page.changes.filter((change) => selected.some(({ path, grant }) => grant === 'exact'
                    ? change.path === path
            ...
            applyChangePage: async (page, cursor, listener, { signal } = {}) => {
                ...
                if (page.changes.length === 0)
                    applied = { journal: page.journal, revision: page.next };
            ...
            walk: async function* (path, { pageSize = 256, maxEntries = 100_000, maxPages = 4_096, maxDepth = 4_096, signal, } = {}) {
                exactFilesystemPageSize(pageSize);
                encodeRequest('filesystem_stat', { path });
            ...
            resumeWalk: async function* (failure, { pageSize, maxEntries, maxPages, maxDepth, signal, } = {}) {
                const token = exactWalkResume(failure instanceof FileWalkOperationError ? failure.resume : failure);
                ...
                if (tightened.maxEntries > token.maxEntries ||
        ...
        credentials: {
            ...
            remove: async (observed, key) => {
                const exactKey = exactCredentialKey(key);
                const receipt = expect(await session.call('credential_remove', { observed, key: exactKey }), 'credential_write');
                if (receipt.key !== exactKey || receipt.observed !== observed)
                    throw new CredentialWriteProtocolError({ key: exactKey, observed }, receipt);
                return receipt.revision;
            },
            ...
            recoverRemove: async (failure, options = {}) => {
                if (!(failure instanceof CredentialRemoveOperationError))
                    throw new TypeError('credential removal recovery requires CredentialRemoveOperationError');
                const current = await api.credentials.read(failure.key, options);
                if (current.revision === failure.observed + 1 && current.value === null)
                    return current.revision;
                current.value?.fill(0);
                throw new Error(`credential ${failure.key} no longer proves the ambiguous removal; no mutation was replayed`);
            },
        },
        postgres: {
            ...
            cancel: async (lease, query) => {
                const receipt = expect(await session.call('postgres_query_cancel', { lease, query }), 'postgres_state');
                if (receipt.lease !== lease || receipt.query !== query) {
                    throw new PostgresStateProtocolError(lease, query, receipt.lease, receipt.query);
                }
                return receipt.state;
            ...
            closeQueryOnce: async (operation, lease, query) => {
                expect(await session.call('postgres_query_close_once', { operation, lease, query }), 'done');
            },
    ...
    api.postgres.closeQueryRecoverable = async (lease, query, { operation: asked } = {}) => {
        ...
        const recovery = { version: 1, kind: 'query', operation, lease, query };
        try {
    ...
    api.postgres.closeLeaseRecoverable = async (lease, { operation: asked } = {}) => {
        const operation = terminalInputOperation(asked);
        const recovery = { version: 1, kind: 'lease', operation, lease };
        try {
    ...
    api.postgres.recoverClose = async (candidate) => {
        const recovery = candidate instanceof PostgresCloseOperationError ? candidate.recovery : candidate;
    ...
    const postgresPages = async function* (lease, query, options = {}) {
        const { cursor: initialCursor, columns: initialColumns, cursors: initialCursors = [], pages: initialPages = 0, maxPages = 4_096, signal, } = options;
        if (!Number.isSafeInteger(maxPages) || maxPages < 1 || maxPages > 1_000_000)
        ...
        for (;;) {
            ...
            try {
                ...
                if (cause instanceof ExtensionError ||
                    ...
                    throw cause;
                throw new PostgresPagesOperationError({ version: 1, lease, query, cursor, columns, cursors: [...cursors], pages, maxPages }, cause);
            }
            if (columns === null)
                columns = [...page.columns];
            else if (page.columns.length !== columns.length ||
                page.columns.some((column, index) => column !== columns[index]))
                throw new PostgresPageShapeProtocolError('columns changed within one query page stream', page);
            if (cursor !== null)
                cursors.add(cursor);
            const next = page.next_cursor ?? null;
            if (next !== null && cursors.has(next))
                throw new PostgresPageShapeProtocolError('next cursor cycles within one query page stream', page);
            cursor = next;
            pages += 1;
            yield page;
            if (cursor === null)
    ...
    api.terminal.reconcileWriteFailure = async (failure, { lines } = {}) => {
        const candidate = failure instanceof TerminalOperationError &&
        ...
        if (!candidate ||
            ...
            throw new TypeError('terminal input reconciliation requires an ambiguous write-input error or recovery token');
        }
        ...
        const receipt = await api.terminal.writeInput(before.slot, before.generation, before.revision, input, { writer: candidate.writer, sequence: candidate.sequence });
