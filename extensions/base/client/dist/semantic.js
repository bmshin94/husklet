...
export function semanticText(tree) {
    ...
    const writeNode = (entry, depth, reserve) => {
        ...
        const close = '</node>';
        if (!append(`<node id="${escapeXml(id.value)}" role="${escapeXml(role.value)}" disabled="${entry.disabled === true}" destructive="${entry.destructive === true}" actions="${actions}">`, reserve + bytes(close) + 14))
            return;
        if (entry.label != null) {
        ...
        if (cut)
            append('<truncated/>', reserve + bytes(close));
        append(close);
    };
    const close = '</pane>';
