...
export function semanticText(tree) {
  if (
    !Number.isSafeInteger(tree?.generation) ||
    tree.generation < 0 ||
  ...
  const append = (text, reserve = 0) => {
    ...
    if (used + size + reserve > XML_LIMIT) {
      cut = true;
      return false;
    }
    output += text;
  ...
  const writeNode = (entry, depth, reserve) => {
    if (!entry || typeof entry !== 'object' || nodes >= NODE_LIMIT || depth >= DEPTH_LIMIT) {
      cut = true;
      return;
    }
    nodes += 1;
    ...
    const role = boundedText(entry.role ?? '');
    const close = '</node>';
    if (
      !append(
        `<node id="${escapeXml(id.value)}" role="${escapeXml(role.value)}" disabled="${entry.disabled === true}" destructive="${entry.destructive === true}" actions="${actions}">`,
        reserve + bytes(close) + 14,
      )
    )
      return;
    if (entry.label != null) {
      ...
      append(
        `<label${label.truncated ? ' truncated="true"' : ''}>${escapeXml(label.value)}</label>`,
        reserve + bytes(close) + 14,
      );
    }
    if (entry.value != null) {
      ...
      append(
        `<value${entry.redacted === true ? ' redacted="true"' : ''}${field.truncated ? ' truncated="true"' : ''}>${escapeXml(field.value)}</value>`,
        reserve + bytes(close) + 14,
      );
    }
    for (const child of Array.isArray(entry.children) ? entry.children : []) {
      if (cut) break;
      writeNode(child, depth + 1, reserve + bytes(close));
    }
  ...
  const close = '</pane>';
  append(
    `<pane slot="${attr(tree?.slot ?? '')}" generation="${tree.generation}" revision="${tree.revision}" truncated="${tree?.truncated === true}">`,
    bytes(close) + 14,
  );
