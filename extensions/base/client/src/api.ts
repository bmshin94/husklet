...
export type ReadonlyWorkspaceEnvironmentGrant = {
  readonly [Operation in keyof WorkspaceEnvironmentGrant]: readonly Readonly<
    WorkspaceEnvironmentGrant[Operation][number]
  >[];
};
...
export declare class StateWriteProtocolError extends Error {
  ...
  readonly received: Readonly<{ observed: string; identity: string }>;
}
...
export declare class TerminalInputReconciliationProtocolError extends Error {
  readonly before: Readonly<{ slot: string; generation: number; revision: number }>;
...
export interface WorkspaceApi {
  ...
  terminal: {
    ...
    reconcileWriteFailure(
      failure: TerminalOperationError | TerminalInputRecoveryToken,
    ...
    writeObservedAndWaitForQuietText(
      ...
      | { changed: false; settled: false; replaced: false; before: PaneText }
    >;
  ...
  files: {
    ...
    resumeWalk(
      failure: FileWalkOperationError | FileWalkResumeToken,
      options?: {
        pageSize?: number;
        maxEntries?: number;
        maxPages?: number;
    ...
    resumeChunks(
      failure: FileChunkOperationError | FileChunkResume,
      options?: {
        chunkBytes?: number;
        maxBytes?: number;
        maxChunks?: number;
  ...
  state: {
    ...
    recoverJsonWrite<T>(
      failure: StateWriteOperationError,
      codec: StateCodec<T>,
    ): Promise<JsonState<T>>;
