...
export class Session {
  ...
  #containers = freezeGrant({ selectors: [], create: false }) as ReadonlyContainerGrant;
  #images = freezeGrant({
    read: [],
    use: [],
    pull: [],
  ...
  constructor(
    ...
    this.#closedPromise = new Promise<Error>((resolve) => {
  ...
  onEvent(listener: (event: HostEvent, channel: number) => void | Promise<void>) {
    ...
    return () => {
      if (this.#events.get(listener) === generation) this.#events.delete(listener);
    };
  }
  ...
  #validateFilesystemJournal(inventory: FileInventory) {
    ...
    for (const entry of inventory.entries) {
      const permitted = this.#filesystem.read.some((selector) => {
        if ('exact' in selector) return selector.exact === entry.path;
        return entry.path === selector.subtree || entry.path.startsWith(`${selector.subtree}/`);
      });
      if (!permitted) {
        throw new TypeError(
          `filesystem snapshot path ${JSON.stringify(entry.path)} is outside the connected read grant`,
        );
      }
    }
  ...
  #greet(frame) {
    ...
    if (
      resources.containers.selectors.length > 0 &&
      !any([
        'containers:read',
        'containers:execute',
        'containers:input',
      ...
      throw new TypeError(
        'host greeting discloses container selectors without container authority',
      );
    ...
    for (const [operation, capability] of [
      ...
      if (resources.images[operation].length > 0 && !holds(capability))
        throw new TypeError(
          `host greeting discloses image ${operation} selectors without ${capability}`,
        );
    ...
    this.#containers = freezeGrant(resources.containers) as ReadonlyContainerGrant;
