...
function terminalInputOperation(operation?: string): string {
  ...
  if (
    ...
    throw new TypeError(
      'terminal input operation must be 16 through 128 lowercase hexadecimal characters',
    );
  }
  return value;
}
...
export class TerminalCommandInputOperationError extends Error {
  ...
  constructor(command, operation, offset, input, close, cause) {
    super(
      `terminal command ${close ? 'input close' : 'input write'} at offset ${offset} may have committed: ${cause instanceof Error ? cause.message : String(cause)}`,
      { cause },
    );
...
function terminalCommandInputFailure(command, operation, offset, input, close, cause) {
  const error = new TerminalCommandInputOperationError(
    command,
    operation,
    offset,
    input,
  ...
  terminalCommandInputRecoveries.set(
    error,
    Object.freeze({
...
export class PostgresOperationProtocolError extends Error {
  ...
  readonly receivedOperation;
  readonly expectedLease;
  readonly receivedLease;

  constructor(phase, expectedOperation, receivedOperation, expectedLease, receivedLease) {
    super(
      `host returned PostgreSQL ${phase} authority for another ${expectedLease === undefined ? 'operation' : 'operation or lease'}`,
    );
...
export class PostgresCancelOperationError extends Error {
  ...
  constructor(recovery, cause) {
    super(
      `postgres query cancellation may have committed: ${cause instanceof Error ? cause.message : String(cause)}`,
      { cause },
    );
...
export class CredentialSetOperationError extends Error {
  ...
  constructor(key, observed, value, cause) {
    super(
      `credential ${key} update after revision ${observed} may have committed: ${cause instanceof Error ? cause.message : String(cause)}`,
      { cause },
    );
...
export class ImagePullStartProtocolError extends Error {
  readonly expectedReference;
  readonly receivedReference;
...
export class ImagePullStatusProtocolError extends Error {
  readonly expected;
  readonly received;
  constructor(expected, received) {
    super('host returned image-pull status for another job or reference');
    this.name = 'ImagePullStatusProtocolError';
    this.expected = Object.freeze({ ...expected });
    this.received = Object.freeze({ job: received.job, reference: received.reference });
  }
}
...
export class IncompleteCatalogueError extends Error {
  ...
  constructor(received) {
    super(
      `extension catalogue is incomplete after ${received} entries; no continuation is available`,
    );
...
function credentialAbort(signal: AbortSignal) {
  const error = new Error('credential use was revoked', { cause: signal.reason });
  error.name = 'AbortError';
  return error;
}

function acquisitionAbort(signal?: AbortSignal) {
...
function exactExecutionSignal(signal: string) {
  if (
...
function exactExecutionCancellation(timeoutMs: number) {
...
function exactTerminalReadLines(lines: number | undefined) {
...
export class TerminalCloseOperationError extends Error {
  ...
  constructor(slot, generation, revision, cause) {
    super(
      `terminal pane ${slot} at generation ${generation} may have closed: ${cause instanceof Error ? cause.message : String(cause)}`,
      { cause },
    );
...
export class SemanticActionOperationError extends Error {
  ...
  constructor(before, action, operation, observed, cause) {
    super(
      `semantic action ${action.action} on node ${action.node} may have committed: ${cause instanceof Error ? cause.message : String(cause)}`,
    );
...
export class PaneInventoryChangedError extends Error {
  ...
  constructor(attempts, before, after) {
    super(
      `pane inventory changed during ${attempts} bounded snapshot attempt${attempts === 1 ? '' : 's'}`,
    );
    this.name = 'PaneInventoryChangedError';
    this.attempts = attempts;
    const cursor = ({ slot, generation, revision, focused }) =>
      Object.freeze({ slot, generation, revision, focused });
...
export class TerminalLayoutChangedError extends Error {
  ...
  constructor(attempts, before, after) {
    super(
      `terminal layout or pane membership changed during ${attempts} bounded snapshot attempt${attempts === 1 ? '' : 's'}`,
    );
...
export class IncompletePaneInventoryError extends Error {
  ...
  constructor(panes) {
    super(
      `stable pane inventory omitted additional panes after observing ${panes.length} identities`,
    );
    this.name = 'IncompletePaneInventoryError';
    this.panes = Object.freeze(
      panes.map(({ slot, generation, revision }) => Object.freeze({ slot, generation, revision })),
    );
  }
}
...
export class FileTextLimitError extends RangeError {
  ...
  constructor(path, identity, total, limit) {
    super(
      `filesystem text ${path} at identity ${identity} exceeds the caller's ${limit} byte limit`,
    );
...
export class FileTextOperationError extends Error {
  ...
  constructor(path, identity, contents, maxBytes, cause) {
    super(
      `filesystem text ${path} at identity ${identity} failed after ${contents.length} acknowledged bytes`,
      { cause },
    );
...
export class FileExtentChangedError extends Error {
  ...
  constructor(path, identity, expectedTotal, actualTotal, offset) {
    super(
      `filesystem file ${path} at identity ${identity} changed total from ${expectedTotal} to ${actualTotal} bytes at offset ${offset}`,
    );
...
function requireFilesystemActive(signal?: AbortSignal) {
  if (signal?.aborted) throw filesystemAbort(signal);
}

function requireStateUpdateActive(signal?: AbortSignal) {
...
function exactExecutionInput(input: string | Iterable<number>) {
  if (typeof input === 'string') {
...
function exactStateIdentity(identity: string) {
  if (identity === 'absent' || /^sha256:[0-9a-f]{64}$/.test(identity)) return identity;
  throw new TypeError(
    'extension state mutation requires the exact identity returned by state.read()',
  );
}
...
function encodeJsonState<T>(value: T, codec: StateCodec<T>) {
  ...
  if (encoded === undefined) throw new TypeError('extension state codec produced no JSON value');
...
export function workspace(session: ClientSession, { signal }: CallOptions = {}): WorkspaceApi {
  ...
  let terminalInputWriter: Promise<{ writer: string; next: number }> | undefined;
  const allocateTerminalInput = async (writer?: string, sequence?: number) => {
    ...
    if (writer !== undefined && sequence !== undefined) {
      if (!/^[0-9a-f]{32}$/.test(writer) || !Number.isSafeInteger(sequence) || sequence < 0) {
        throw new TypeError(
          'terminal input writer must be 32 lowercase hex characters and sequence nonnegative',
        );
      }
  ...
  const exactTabs = <Tabs extends { id: string }[]>(tabs: Tabs) => {
  ...
  const exactWorkspaceConfiguration = <Configuration extends { name: string }>(
    ...
    if (configuration.name !== name) {
      throw new TypeError(
        `host returned workspace configuration for ${configuration.name}, expected ${name}; no workspace state was assumed`,
      );
    }
  ...
  const exactExecution = (execution: ExecutionSummary, id: string, operation: string) => {
    ...
    const timestamp = (value: number | null, name: string) => {
  ...
  const api = {
    ...
    create: async (configuration) =>
      exactWorkspaceConfiguration(
        expect(
          await session.call('workspace_create', { configuration }),
    ...
    restart: (name, generation) =>
      done('workspace_restart', {
        name,
        generation: immutableIdentity(generation, [32], 'workspace generation'),
      }),
    notifications: {
    ...
    containers: {
      list: async () => {
        ...
        if (new Set(containers.map(({ id }) => id)).size !== containers.length) {
          throw new TypeError(
            'host returned duplicate immutable container identities; no container selection was assumed',
          );
        }
        return containers;
      },
      ...
      resumeExecutionStreaming: async (
        id,
        {
          after = 0,
          expectedContainerId,
      ...
      resumeJsonLinePages: async (id, configuration, onPage) => {
        ...
        try {
          const result = await api.containers.resumeExecutionStreaming(
            id,
            options,
            async (page) => {
      ...
      closeExecutionStdin: (id) =>
        done('execution_close_input', {
          id: immutableIdentity(id, [32], 'execution'),
        }),
      pipeExecutionStdin: async (
        id,
        source,
        { signal, close = true }: { signal?: AbortSignal; close?: boolean } = {},
      ) => {
        const executionId = immutableIdentity(id, [32], 'execution');
        if (
          source === null ||
    ...
    images: {
      ...
      inspect: async (reference) => {
        ...
        if (/^sha256:[0-9a-f]{64}$/.test(reference) && image.id !== reference) {
          throw new TypeError(
            `host returned image ${image.id}, expected ${reference}; no image state was assumed`,
          );
        }
        return image;
      },
      pull: async (
        ...
        try {
          for (;;) {
            ...
            if (status.job !== job) throw new ImagePullStatusProtocolError({ job }, status);
    ...
    volumes: {
      ...
      inspect: async (name) => {
        ...
        if (volume.name !== name) {
          throw new TypeError(
            `host returned volume ${volume.name}, expected ${name}; no volume state was assumed`,
          );
        }
        return volume;
      },
    ...
    terminal: {
      ...
      commandCancel: async (
        command,
        { signal: commandSignal = 'SIGTERM', timeoutMs = 5_000 } = {},
      ) => {
        command = exactTerminalCommand(command);
        ...
        if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 30_000) {
          throw new RangeError(
            'terminal command cancellation timeout must be between 1 and 30000ms',
          );
        }
        return sameTerminalCommand(
          command,
      ...
      commandWrite: async (
        command,
        input,
        { operation: askedOperation, offset = 0 }: { operation?: string; offset?: number } = {},
      ) => {
        command = exactTerminalCommand(command);
        const contents = exactExecutionInput(input);
        const operation = terminalInputOperation(askedOperation);
      ...
      resumeCommandText: async (
        ...
        if (
          !Number.isSafeInteger(consumedPages) ||
          consumedPages < 0 ||
      ...
      read: async (slot, lines) =>
        exactPane(
          expect(
            await session.call('terminal_read_pane', {
              slot,
        ...
        for (const pane of inventory.panes) {
          if (pane.kind === 'terminal') {
            const snapshot = expect(
              await session.call('terminal_read_pane', { slot: pane.slot, lines }),
              'text',
            );
            if (
              snapshot.slot !== pane.slot ||
            ...
            const snapshot = expect(
              await session.call('pane_semantic_read', { slot: pane.slot }),
              'semantics',
            );
            if (
              snapshot.slot !== pane.slot ||
    ...
    files: {
      ...
      changes: async ({ journal, revision: after }, limit = 256) => {
        ...
        if (
          ...
          page.changes.some(
            (change, index) =>
              change.cursor.journal !== page.journal ||
      ...
      watchLatestChanges: async (
        ...
        const running = (async () => {
          try {
            for await (const page of api.files.changePages({
              ...
              const accumulated: FileChangePage = {
                ...
                changes: [...changed.values()].sort(
                  (left, right) => left.cursor.revision - right.cursor.revision,
                ),
            ...
            for (const generation of pending.values()) generation.abort(stopped.signal.reason);
      ...
      readLink: async (path) =>
        expect(await session.call('filesystem_read_link', { path }), 'contents'),
      ...
      resumeText: async (failure, { signal }: { signal?: AbortSignal } = {}) => {
        ...
        if (
          !Number.isSafeInteger(failure.maxBytes) ||
          failure.maxBytes < 1 ||
        ...
        try {
          for await (const range of api.files.readChunks(failure.path, {
            ...
            if (
              range.total > failure.maxBytes ||
              contents.length + range.contents.length > failure.maxBytes
            ) {
              throw new FileTextLimitError(
                failure.path,
                failure.identity,
                range.total,
                failure.maxBytes,
          ...
          if (
            ...
            throw new FileTextOperationError(
              failure.path,
              failure.identity,
              contents,
              failure.maxBytes,
      ...
      writeTextObserved: (observed, contents) => {
        ...
        return api.files.writeObserved(
          observed.path,
          observed.identity,
    ...
    state: {
      ...
      updateJson: async (
        codec,
        update,
        { attempts = 4, signal: updateSignal }: { attempts?: number; signal?: AbortSignal } = {},
      ) => {
        if (!Number.isSafeInteger(attempts) || attempts < 1 || attempts > 16)
          throw new RangeError(
            'extension state JSON update attempts must be an integer from 1 through 16',
          );
    ...
    credentials: {
      ...
      withValue: async (
        ...
        consumer: (
          value: Uint8Array,
          context: { revision: number; signal: AbortSignal },
        ) => void | Promise<void>,
        ...
        if (hostSession.signal.aborted) revokeSession();
        else hostSession.signal.addEventListener('abort', revokeSession, { once: true });
        const timer = setTimeout(
          () => controller.abort(new Error(`credential lease expired after ${maxLifetimeMs}ms`)),
          maxLifetimeMs,
        );
        timer.unref?.();
        ...
        try {
          ...
          const consuming = Promise.resolve().then(() =>
            consumer(exposed!, { revision: credential.revision, signal: controller.signal }),
          );
      ...
      recoverSet: async (failure, options: CallOptions = {}) => {
        ...
        try {
          const same =
            current.revision === failure.observed + 1 &&
            source?.length === failure.value.length &&
            source.every((byte, index) => byte === failure.value[index]);
          if (!same)
            throw new Error(
              `credential ${failure.key} no longer matches the ambiguous update; no mutation was replayed`,
            );
    ...
    postgres: {
      ...
      startOnce: async (lease, query) => {
        const outcome = expect(
          await session.call('postgres_query_start_once', { lease, query }),
          'postgres_start',
        );
  ...
  api.postgres.cancelRecoverable = async (lease, query) => {
    const recovery = { version: 1, lease, query } as const;
    try {
  ...
  api.postgres.recoverCancel = async (candidate) => {
    const recovery =
      candidate instanceof PostgresCancelOperationError ? candidate.recovery : candidate;
  ...
  api.containers.startAndWait = async (id, generation, { timeoutMs = 30_000 } = {}) => {
    ...
    let observedRunning: ContainerSummary | undefined;
    let observed;
    let timer;
    ...
    try {
      ...
      clearTimeout(timer);
      if (startReplyLost) await stop().catch(() => {});
      else await stop();
    }
  };
  api.containers.stopAndWait = async (id, generation, { timeoutMs = 30_000 } = {}) => {
    ...
    let observedExited: ContainerSummary | undefined;
    let observed;
    let timer;
    ...
    try {
      ...
      clearTimeout(timer);
      if (stopReplyLost) await stopWatching().catch(() => {});
  ...
  api.containers.removeAndWait = async (id, generation, { timeoutMs = 30_000 } = {}) => {
    ...
    try {
      ...
      clearTimeout(timer);
      if (removeReplyLost) await stopWatching().catch(() => {});
      else await stopWatching();
    }
  };
  api.containers.restartAndWait = async (id, generation, { timeoutMs = 30_000 } = {}) => {
    ...
    let observed;
    let observedRestarted: ContainerSummary | undefined;
    ...
    try {
      ...
      clearTimeout(timer);
      if (restartReplyLost) await stopWatching().catch(() => {});
      else await stopWatching();
    }
  };
  ...
  api.watchTerminal = (listener) =>
    watch('terminal', 'terminal', (tabs) => listener(exactTabs(tabs as TabSummary[])), 'terminal');
  ...
  api.terminal.readAllStable = async ({ lines, attempts = 3, signal: readSignal } = {}) => {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      try {
        ...
        if (error instanceof PaneChangedError && attempt < attempts) continue;
        throw error;
      }
      ...
      if (unchanged) return readable;
      if (attempt === attempts) throw new PaneInventoryChangedError(attempts, before, after);
    }
  ...
  api.terminal.readLayoutStable = async ({ lines, attempts = 3, signal: readSignal } = {}) => {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      try {
        ...
        if (
          (error instanceof PaneChangedError || error instanceof PaneInventoryChangedError) &&
          attempt < attempts
        )
          continue;
        throw error;
      ...
      const visit = (node, tab) => {
        if (node.kind === 'pane') membership.set(node.pane.slot, tab);
        else {
          visit(node.first, tab);
          visit(node.second, tab);
        }
      };
      for (const tab of after.tabs) visit(tab.root, tab.id);
      const panesMatch =
        membership.size === readable.panes.length &&
  ...
  const writeAndWait = async (
    ...
    input: string | Iterable<number>,
    {
      ...
      timeoutMs = 30_000,
      signal,
    }: { lines?: number; timeoutMs?: number; signal?: AbortSignal } = {},
    ...
    let inputAuthority: { writer: string; sequence: number } | undefined;
    try {
      ...
      for (;;) {
        ...
        if (after.generation === generation && after.revision === revision) continue;
  ...
  api.terminal.writeAndWait = (slot, generation, revision, input, options) =>
    writeAndWait(slot, generation, revision, input, options) as ReturnType<
      WorkspaceApi['terminal']['writeAndWait']
    >;
  ...
  api.terminal.writeObservedAndWaitForQuietText = async (
    before,
    input,
    { lines, quietMs = 150, timeoutMs = 30_000, signal } = {},
  ) => {
    ...
    if (!first.changed) return { ...first, settled: false, replaced: false };
  ...
  api.terminal.pinTabAndWait = async (tab, pinned = true, { timeoutMs = 30_000 } = {}) => {
    ...
    if (typeof pinned !== 'boolean') throw new TypeError('terminal pin state must be boolean');
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 30_000) {
      throw new RangeError('terminal pin wait timeout must be between 1 and 30000ms');
    }
    let observed;
    let authorityIssued = false;
    const inventory = new Promise<TabSummary>((resolve, reject) => {
      observed = (tabs) => {
        if (!authorityIssued) return;
        const current = tabs.find((candidate) => candidate.id === tab);
        if (!current) reject(new Error(`terminal tab ${tab} disappeared while pinning`));
        else if (current.pinned === pinned) resolve(current);
      };
    });
  ...
  api.terminal.actAndWait = async (
    slot,
    action,
    { lines, timeoutMs = 30_000, signal, operation } = {},
  ) => {
    if (typeof slot !== 'string' || slot.length === 0)
    ...
    let abort;
    try {
      if (operation === undefined) await scoped.terminal.act(slot, action);
      else await scoped.terminal.actOnce(operation, slot, action);
      const change = await Promise.race([
        observed,
        new Promise((resolve) => {
          timer = setTimeout(() => resolve(null), timeoutMs);
  ...
  api.terminal.recoverSemanticAction = async (candidate) => {
    const recovery =
      candidate instanceof SemanticActionOperationError ? candidate.recovery : candidate;
    if (recovery?.version !== 1) {
  ...
  api.extensions.recoverCommit = async (failure, { timeoutMs = 30_000, signal } = {}) => {
    ...
    try {
      ...
      throw new Error(
        `extension ${failure.candidate.name} commit cannot be recovered because its acquisition history is unavailable and durable extension state does not match the reviewed candidate`,
        { cause },
      );
    }
    const deadline = Date.now() + timeoutMs;
  ...
  api.extensions.cancelAcquisitionAndWait = async (
    job,
    revision,
    { timeoutMs = 30_000, signal } = {},
  ) => {
    ...
    if (!Number.isSafeInteger(revision) || revision < 0)
      throw new TypeError(
        'extension acquisition cancellation requires a nonnegative safe integer revision',
      );
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 30_000)
      throw new RangeError(
        'extension acquisition cancellation timeout must be between 1 and 30000ms',
      );
    if (signal?.aborted) throw acquisitionAbort(signal);
    const scoped = signal ? api.withSignal(signal) : api;
    let pending = false;
    ...
    const next = (remaining) =>
      pending
        ...
        : new Promise<boolean>((resolve, reject) => {
            ...
            const done = (value, error?: unknown) => {
              clearTimeout(timer);
              signal?.removeEventListener('abort', abort);
              wake = undefined;
    ...
    const stop = await api.watchExtensionAcquisitions((change) => {
      if (change.job !== identity || change.revision <= revision) return;
      pending = true;
      wake?.();
      wake = undefined;
    });
    ...
    try {
      ...
      if (current.state === 'cancelled')
        return { changed: true, status: { ...current, state: 'cancelled' as const } };
      if (
      ...
      for (;;) {
        ...
        if (status.revision > revision) {
          if (status.state !== 'cancelled')
            throw new Error(
              `extension acquisition finished as ${status.state} while cancellation was pending`,
            );
          return { changed: true, status: { ...status, state: 'cancelled' as const } };
        }
        const remaining = deadline - Date.now();
        if (remaining <= 0) return { changed: false, job: identity, revision };
        const advanced = await next(remaining);
        if (!advanced) return { changed: false, job: identity, revision };
        pending = false;
      }
    } finally {
      clearTimeout(timer);
