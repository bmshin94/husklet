...
test(
  'same-id execution replacement fences detail output wait and consent',
  { timeout: 8_000 },
  async () => {
    const directory = await mkdtemp(join(tmpdir(), 'husklet-execution-lifecycle-'));
    ...
    const server = net.createServer((socket) => {
      ...
      socket.on('data', (chunk) => {
        for (const frame of reader.take(chunk)) {
          ...
          if (call === 'execution_list') {
          ...
          } else if (call === 'execution_inspect' || call === 'execution_wait')
            payload = {
              reply: 'execution',
              with: {
                id,
            ...
            payload = {
              reply: 'logs',
              with: {
                stdout: [...new TextEncoder().encode('stale-output')],
                stderr: [],
                truncated: false,
    ...
    try {
      ...
      raceSession = await connect({ path: socketPath });
      const framed = workspace(session);
      const race = workspace(raceSession);
      const mutations = [];
      stage = host();
      stage.render(
        h(Top, {
          api: {
            ...
            containers: {
              ...framed.containers,
              execution: race.containers.execution,
              executionLogs: race.containers.executionLogs,
              waitExecution: race.containers.waitExecution,
            },
            subscribe: undefined,
            unsubscribe: undefined,
          },
          executionDetails: new ExecutionDetailsSource(async (mutation) =>
      ...
      assert.equal(
        stage.frames
          .slice(start)
          .flatMap((frame) => frame.patches)
          .some((patch) => patch.SetProp?.value?.Text === `Send SIGTERM to execution ${id}?`),
        false,
      );

      invoke(stage, 'Details');
      await until(() => lengths(mutations) === 1);
      ...
      invoke(stage, 'Details');
      await until(() => lengths(mutations) === 2);
...
function lengths(mutations) {
  return mutations.filter((mutation) => mutation.Length).length;
}
...
async function until(done) {
  ...
  while (!done()) {
    if (Date.now() >= deadline) throw new Error('execution lifecycle did not settle');
    await delay(10);
  }
}
