...
test(
  'same-name volume generation replacement invalidates detail authority',
  { timeout: 5_000 },
  async () => {
    ...
    const server = net.createServer((socket) => {
      ...
      socket.write(
        encode({
          ...
          payload: {
            protocol: 1,
            extension: 'volume-detail-lifecycle-test',
            granted: ['volumes:read', 'volumes:write'],
          },
        }),
      );
                ...
                volumes: [
                  {
                    name,
                    driver: 'local',
                    generation: lists === 1 ? oldGeneration : newGeneration,
                  },
                ],
                truncated: false,
              },
            };
          } else if (call === 'volume_inspect') {
            inspections += 1;
            payload = {
              reply: 'volume',
              with: {
                name,
                driver: inspections === 1 ? 'old-driver' : 'new-driver',
                generation: inspections === 1 ? oldGeneration : newGeneration,
              },
            };
          }
    ...
    try {
      ...
      stage.render(
        h(Top, {
          api: {
            ...framed,
            volumes: { ...framed.volumes, inspect: inspectionApi.volumes.inspect },
            subscribe: undefined,
            unsubscribe: undefined,
          },
          volumeDetails: new VolumeDetailsSource(async (mutation) => mutations.push(mutation)),
          initial: { containers: [], executions: [], images: [], networks: [] },
        }),
      );
      invoke(stage, 'Volumes');
      await until(() => labelled(stage, name));
      ...
      assert.ok(
        ...
        'replacement unmounts prior generation and consent',
      );
      assert.equal(
        patches.some((patch) => patch.SetProp?.value?.Text === 'old-driver'),
        false,
      );
      assert.equal(
        value(stage, 'Volume name'),
        'draft-volume',
        'independent creation input survives replacement',
      );
...
function change(stage, placeholder, next) {
  const node = stage.frames
    ...
    .at(-1)?.SetProp.id;
  assert.ok(stage.surface.dispatch({ trigger: 'Change', node, id: `${node}:Change`, value: next }));
}
function value(stage, placeholder) {
  const node = stage.frames
    .flatMap((frame) => frame.patches)
    .filter(
      (patch) => patch.SetProp?.prop === 'Placeholder' && patch.SetProp.value?.Text === placeholder,
...
async function until(done) {
  ...
  while (!done()) {
    if (Date.now() >= deadline) throw new Error('volume detail lifecycle did not settle');
