...
test(
  'volume inventory removes stale generation authority across real framed refresh states',
  { timeout: 5_000 },
  async () => {
    ...
    const server = net.createServer((socket) => {
      ...
      socket.write(
        encode({
          ...
          payload: {
            protocol: 1,
            extension: 'volume-list-resource-test',
              ...
              with: {
                volumes:
                  attempts === 3
                    ...
                    : [
                        {
                          name: attempts === 1 ? 'stale-cache' : 'current-cache',
                          driver: 'local',
