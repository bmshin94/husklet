...
import { IMAGE_DETAIL_SOURCE, ImageDetailsSource } from '../dist/model.js';
import { host } from './host.js';

test(
  'authoritative image replacement invalidates detail and removal consent',
  { timeout: 5_000 },
  async () => {
    ...
    const server = net.createServer((socket) => {
      ...
      socket.write(
        encode({
          ...
          payload: {
            protocol: 1,
            extension: 'image-detail-lifecycle-test',
        ...
        for (const frame of reader.take(chunk)) {
          ...
          if (call === 'image_list') {
            ...
            payload = {
              ...
              with: {
                images: [
                  {
                    id: digest,
                    reference: listAttempts === 1 ? 'old/image:1' : 'new/image:2',
                    size: 1024,
                    created: listAttempts,
                  },
                ],
                truncated: false,
              },
            ...
            payload = {
              ...
              with: {
                id: digest,
                references: [inspectAttempts === 1 ? 'old/image:1' : 'new/image:2'],
                created: 'now',
                size: 1024,
                os: 'linux',
                architecture: 'amd64',
                entrypoint: [],
                command: [],
                working_directory: inspectAttempts === 1 ? '/old-work' : '/new-work',
                user: '',
              },
            };
          }
    ...
    try {
      ...
      assert.ok(labelled(stage, `Removing old/image:1 (${digest.slice(0, 12)}) cannot be undone.`));
      ...
      assert.ok(
        ...
        'refresh unmounts prior image detail and consent',
      );
      assert.equal(
        refreshPatches.some(
          (patch) =>
            patch.SetProp?.value?.Text ===
            `Removing old/image:1 (${digest.slice(0, 12)}) cannot be undone.`,
        ),
        false,
        'removal consent does not remount for a reused digest',
      );
      assert.equal(
        refreshPatches.some((patch) => patch.SetProp?.value?.Text === '/old-work'),
        false,
      ...
      assert.equal(
        ...
        1,
        'inventory replacement cannot republish prior detail',
      );
