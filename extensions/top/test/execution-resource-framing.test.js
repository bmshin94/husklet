...
test(
  'execution detail traverses every ResourceState over real framing and drops stale detail',
  { timeout: 5_000 },
  async () => {
    ...
    const server = net.createServer((socket) => {
      ...
      socket.write(
        encode({
          ...
          payload: {
            protocol: 1,
            extension: 'execution-resource-test',
            granted: [
              'containers:read',
              'containers:create',
              'containers:execute',
      ...
      socket.on('data', (chunk) => {
        for (const frame of reader.take(chunk)) {
          ...
          if (frame.payload.call === 'execution_inspect') {
            ...
            frame.payload.with?.topic === 'executions'
          ) {
              ...
              if (observation)
                socket.write(
                  encode({
                    channel: 98,
                    kind: KIND.event,
                    payload: { snapshot: 'executions', of: observation },
                  }),
                );
            },
    ...
    try {
      ...
      assert.equal(
        ...
        1,
        'failed refresh cannot republish stale detail rows',
      );
      ...
      assert.ok(
        labelled(
          stage,
          `SIGTERM completed and execution ${executionId.slice(0, 12)} was observed exited.`,
        ),
      );
    } finally {
      session?.close();
