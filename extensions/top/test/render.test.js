...
test('extension discovery searches, filters, reports result counts, and clears a no-match state', async () => {
  ...
  assert.deepEqual(placeholderProperty(stage, 'Search extensions', 'Width'), {
    Bounds: { minimum: { Chars: 24 }, maximum: { Chars: 34 } },
  });
...
for (const updating of [false, true]) {
  test(`extension ${updating ? 'update' : 'install'} independently narrows container and filesystem authority`, async () => {
    ...
    assert.equal(
      ...
      1,
      'decision actions retain one compact semantic group',
    );
    assert.ok(
      sharedAncestor(stage, ['Review decision · 8/13 selected', commitLabel], 'Column'),
      'status and actions share the fixed decision footer',
    );
    assert.equal(
      sharedAncestor(stage, ['Review decision · 8/13 selected', commitLabel], 'Row'),
      undefined,
      'the decision summary stays above rather than competing with the actions',
    );
...
test('volume and network panels render bounded real inventories and controls', () => {
  ...
  assert.ok(
    ancestorTags(networkStage, 'Manage network').includes('CardContent'),
    'the primary connection-management action appears in the summary band',
  );
  assert.equal(ancestorTags(networkStage, 'Manage network').includes('CardActions'), true);
  assert.deepEqual(ancestorTags(networkStage, 'Manage network').slice(0, 3), [
    'CardActions',
    'Row',
    'Responsive',
  ]);
  assert.deepEqual(
    taggedProperty(networkStage, 'Manage network', 'InlineButton', 'Enabled')?.Flag,
    true,
...
test('network inspection exposes loading, retry, empty and domain-specific details', async () => {
  ...
  assert.ok(labelled(stage, 'Loading network…'));
  const managingNode = labelled(stage, 'Loading network…').SetProp.id;
  assert.ok(
    stage.frames
      .flatMap((frame) => frame.patches)
      .some(
  ...
  invoke(stage, 'Retry network details');
  await settled();
  await settled();
...
test('container controls follow the real daemon lifecycle states', async () => {
  ...
  assert.equal(taggedProperty(stage, 'Start', 'InlineButton', 'Tone')?.Tone, 'Accent');
...
test('volume and network mutations expose danger only on final confirm and cancel safely', async () => {
  ...
  invoke(networks, 'Manage network');
  await settled();
  await settled();
