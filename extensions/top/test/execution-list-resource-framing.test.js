...
test(
  'execution catalogue removes stale actions across framed loading, failure, retry and empty states',
  { timeout: 5_000 },
  async () => {
                      ...
                      : [
                          {
                            ...
                            finished_at_ms: null,
                            pid: 42,
                            command: [attempts === 1 ? 'stale-command' : 'current-command'],
                            user: 'root',
                          },
                        ],
    ...
    try {
      ...
      assert.ok(
        ...
        'loading unmounts stale execution cards',
      );
      await until(() => labelled(stage, 'execution inventory unavailable'));
      assert.equal(
        refreshPatches.some((patch) => patch.SetProp?.value?.Text === 'Terminate'),
        false,
      );
      assert.equal(
        refreshPatches.some(
      ...
      assert.ok(
        ...
        'ready state restores actions for current execution inventory',
      );
    } finally {
      stage?.render(null);
